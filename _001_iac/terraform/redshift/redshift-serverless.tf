...
resource "aws_redshiftserverless_namespace" "serverless" {
  depends_on = [
    aws_iam_role_policy_attachment.attach-s3,
    aws_iam_role_policy.redshift-s3-full-access-policy
  ]
...
resource "aws_redshiftserverless_workgroup" "serverless" {
  depends_on = [
    aws_redshiftserverless_namespace.serverless,
    aws_route_table_association.redshift-serverless-rta-az1,
    aws_route_table_association.redshift-serverless-rta-az2,
    aws_route_table_association.redshift-serverless-rta-az3,
    aws_internet_gateway.redshift-serverless-igw
  ]
